...
@section('content')
                    ...
                    <form class="kt-form kt-form--label-right" id="addForm" name="addForm" method="post"
                        ...
                        <div class="modal-body">
                            <div class="form-group">
                                <label for="recipient-name" class="form-control-label">Enter Product/Service</label>
        ...
        <div class="modal fade" id="editModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLongTitle"
            ...
            <div class="modal-dialog" role="document">
                <div class="modal-content">
                    ...
                    <form class="kt-form kt-form--label-right" id="addForm" name="addForm" method="post"
                        action="{{ url('admin/update-project-type') }}" enctype="multipart/form-data">
                        {{ method_field('post') }}
                        @csrf
                        <input type="hidden" name="type_id" id="type_id">
                        <div class="modal-body">
                            <div class="form-group">
                                <label for="recipient-name" class="form-control-label">Enter Product/Service Name</label>
                                <input type="text" class="form-control" placeholder="Enter Name"
                                    value="{{old('name')}}" name="name" id="edit_name" required autofocus>
                            </div>
                        </div>
                        <div class="modal-footer">
                            <button type="button" class="btn btn-outline-danger" id="addClientSubmit"
    ...
    <button type="button" class="btn btn-danger" onclick="deleteConfirmation(` + full.id + `)"><i class="fa fa-trash" aria-hidden="true"></i></button>
                ...
                function deleteConfirmation(id) {
                    swal({
                        ...
                        if (e.value === true) {
                            ...
                            $.ajax({
                                type: 'GET',
                                url: "{{ url('/admin/projecttypedelete') }}/" + id,
                                data: {
                                    _token: CSRF_TOKEN
                                },
                                dataType: 'JSON',
                ...
                jQuery.ajax({
                        method: "get",
                        url: "{{url('admin/edit-project-type')}}" + "/" + sid,
                        data:{
                            "_token": "{{ csrf_token() }}",
                        },
                        success: function(res) {
                            $('.editBtn').removeAttr('disabled');
                            $('#edit_name').val(res.result.project_type);
