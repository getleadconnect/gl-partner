...
@section('content')
    ...
    <div class="row">
            ...
            <div class="modal-dialog" role="document">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="exampleModalLongTitle">Add Sales Person</h5>
                    ...
                    <form class="kt-form kt-form--label-right" id="addForm" name="addForm" method="post"
                        action="{{url('admin/projectowner')}}" enctype="multipart/form-data">
                        @csrf
                        <div class="modal-body">
                            <div class="form-group">
                                <label for="recipient-name" class="form-control-label">Enter Name of the person</label>
                                <input type="text" class="form-control" placeholder="Enter Name of the person"
                                    value="{{ old('name') }}" name="name" id="name" required autofocus>
                            </div>
                        </div>
        ...
        <div class="modal fade" id="editModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLongTitle"
            ...
            <div class="modal-dialog" role="document">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="exampleModalLongTitle">Edit Sales Person</h5>
                        <button type="button" class="close form-close" data-dismiss="modal" aria-label="Close">
                        </button>
                    </div>
                    <form class="kt-form kt-form--label-right" id="editForm" name="editForm" method="post"
                        action="{{ url('admin/projectownerUpdate') }}" enctype="multipart/form-data">
                        {{ method_field('post')}}
                        @csrf
                        ...
                        <div class="modal-body">
                            <div class="form-group">
                                ...
                                <input type="text" class="form-control" placeholder="Enter Name"
                                    ...
                                    id="edit_name" required autofocus>
                            </div>
                        </div>
                        <div class="modal-footer">
                            <button type="button" class="btn btn-outline-danger" id="addClientSubmit"
    ...
    <button type="button" class="btn btn-danger" onclick="deleteConfirmation(` + full.id + `)"><i class="fa fa-trash" aria-hidden="true"></i></button>
                ...
                function deleteConfirmation(id) {
                    swal({
                        ...
                        if (e.value === true) {
                            ...
                            $.ajax({
                                type: 'GET',
                                url: "{{ url('/admin/projectownerdelete') }}/" + id,
                                data: {
                                    _token: CSRF_TOKEN
                                },
                                dataType: 'JSON',
                ...
                $('#example').on('click', '.editBtn', function() {
                    ...
                    jQuery.ajax({
                        method: "get",
                        url: "{{ url('admin/projectownerEdit') }}" + "/" + sid,
                        data: {
                            "_token": "{{ csrf_token() }}",
                        },
